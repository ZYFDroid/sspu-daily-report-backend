...
function myOnLoadEnd(url){
    ...
    if(url.startsWith(my_reportUrl)){
        if(my_autoReport == true){
            ...
            setTimeout(injectDate,333);
        }
    }
}

var lskey_injectionDate = "_zyf_app_injectiondate";
var injectDate = function(){
	var elm = document.getElementById("p1_JieZSJ-inputEl");
	var vvalue = localStorage.getItem(lskey_injectionDate);
	if(elm!=null && vvalue != null){
		elm.value = vvalue;
	}
	if(elm==null){
		setTimeout(injectDate,333);
...
function myAutoQuit(){
    ...
    if(document.getElementsByClassName("f-messagebox-message")[0].innerText.indexOf("提交成功") != -1){
        ...
        ProgressAccessor.hideProgress();
        localStorage.setItem(lskey_injectionDate,document.getElementById("p1_JieZSJ-inputEl").value);
